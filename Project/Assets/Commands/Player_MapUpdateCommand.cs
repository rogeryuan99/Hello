...
public class Player_MapUpdateCommand : Command {
	//type = a:1/10, b:1/100, c:1/1000
	public Player_MapUpdateCommand( string playerId,string authToken, CompleteDelegate completeDelegate, ErrorDelegate errorDelegate){
		Hashtable batchHash = new Hashtable ();
		batchHash.Add ("authKey", authToken);
		
		ArrayList commands = new ArrayList();
		Hashtable command = new Hashtable ();
		command.Add ("action", "player.mapUpdate");
		command.Add ("time", TimeUtils.UnixTime);
		command.Add ("args", new Hashtable () { { "playerId", playerId },{"map", MapMgr.Instance.dumpDynamicData()}});
