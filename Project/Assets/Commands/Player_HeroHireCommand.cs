...
public class Player_HeroHireCommand : Command {
	//type = a:1/10, b:1/100, c:1/1000
	public Player_HeroHireCommand( string playerId,string authToken,string type, CompleteDelegate completeDelegate, ErrorDelegate errorDelegate){
		Hashtable batchHash = new Hashtable ();
		batchHash.Add ("authKey", authToken);
		
		ArrayList commands = new ArrayList();
		Hashtable command = new Hashtable ();
		command.Add ("action", "player.heroHire");
		command.Add ("time", TimeUtils.UnixTime);
		command.Add ("args", new Hashtable () { { "playerId", playerId },{"herotype",type}});
