/*
   2011.11月 8号 下午 4:03	MovePivot V1.0
...
public class MovePivot : EditorWindow {
	...
	float yPos;
	float zPos;
    [MenuItem ("L Tools/Move Pivot")] //Place the Set Pivot menu item in the GameObject menu
    static void Init ()
	{
        MovePivot window = (MovePivot)EditorWindow.GetWindow (typeof (MovePivot));
	...
	void OnGUI() {
		if(obj) {
			if(mesh) {
				xPos = EditorGUILayout.FloatField("X Position:", xPos);
				yPos = EditorGUILayout.FloatField("Y Position:", yPos);
