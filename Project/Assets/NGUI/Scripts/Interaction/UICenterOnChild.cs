...
public class UICenterOnChild : MonoBehaviour
{
	...
	public int minCells = 0;
	private int defaultIndex = -1;
	private bool isJumping = false;
	[HideInInspector]public GameObject hotCellGo = null;
	
	[HideInInspector] public bool recalculateCenterCellOnUpdate = false;
	...
	void Update ()
	{
		...
		center.x += clip.x + this.offset.x;
		center.y += clip.y + this.offset.y;
		...
		for (int i = 0, imax = trans.childCount; i < imax; ++i) {
			...
			if (imax - i < minCells && closest != null) {
				continue;
			}
			if (sqrDist < min) {
				min = sqrDist;
		...
		if(closest != null) hotCellGo = closest.gameObject;
	...
	public void Recenter ()
	{
		...
		if (defaultIndex != -1 && defaultIndex <= trans.childCount - minCells) {
			closest = trans.GetChild (defaultIndex);
		} else {
			for (int i = 0, imax = trans.childCount; i < imax; ++i) {
				Transform t = trans.GetChild (i);
				float sqrDist = Vector3.SqrMagnitude (t.position - offsetCenter);
		...
		if (closest != null) {
			...
			if(isJumping){
				SpringPanel.Begin (mDrag.gameObject, dt.localPosition - offset, float.MaxValue).onFinished = onFinished;
			}else{
