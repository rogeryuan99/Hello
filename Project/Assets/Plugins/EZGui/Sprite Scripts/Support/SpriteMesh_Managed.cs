...
public class SpriteMesh_Managed : ISpriteMesh, IEZLinkedListItem<SpriteMesh_Managed>
{
	...
	private Vector3[] _origialVertices;
	public Vector3[] origialVertices{
		get{
			if(_origialVertices == null) return null;
			else{
				Vector3[] a;	
				a = new Vector3[4];
				a[0] = new Vector3(_origialVertices[0].x,_origialVertices[0].y,_origialVertices[0].z);
				a[1] = new Vector3(_origialVertices[1].x,_origialVertices[1].y,_origialVertices[1].z);
				a[2] = new Vector3(_origialVertices[2].x,_origialVertices[2].y,_origialVertices[2].z);
				a[3] = new Vector3(_origialVertices[3].x,_origialVertices[3].y,_origialVertices[3].z);
				return a;
			}
		}
	}
	...
	public Vector3[] myVertices{
		...
		set{
			if(_origialVertices == null){
				_origialVertices = new Vector3[4];
				_origialVertices[0] = new Vector3(m_vertices[0].x,m_vertices[0].y,m_vertices[0].z);
				_origialVertices[1] = new Vector3(m_vertices[1].x,m_vertices[1].y,m_vertices[1].z);
				_origialVertices[2] = new Vector3(m_vertices[2].x,m_vertices[2].y,m_vertices[2].z);
				_origialVertices[3] = new Vector3(m_vertices[3].x,m_vertices[3].y,m_vertices[3].z);
//				Debug.Log("new originalVertise,[0]="+_origialVertices[0]);
//				Debug.Log("new originalVertise,[1]="+_origialVertices[1]);
//				Debug.Log("new originalVertise,[2]="+_origialVertices[2]);
//				Debug.Log("new originalVertise,[3]="+_origialVertices[3]);
			}
			m_vertices = value;
			UpdateVerts();
		}
