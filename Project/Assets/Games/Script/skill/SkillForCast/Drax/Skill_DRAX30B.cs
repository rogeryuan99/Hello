...
public class Skill_DRAX30B : SkillBase {
	...
	public override IEnumerator Cast(ArrayList objs){
		...
		SkillDef skillDef = SkillLib.instance.getSkillDefBySkillID("DRAX30B");
		int skillDurationTime = skillDef.skillDurationTime;
		
		drax.castSkill("Skill30");
		drax.attackAnimaName = "Skill30B";
		drax.showSkill30EftCallback += showEft;
		drax.showSkill30BodyEftCallback += showBodyEft;
		drax.showSkill30BMusicCallback += showMusic;
		
		drax.addBuff("Skill_DRAX30B", skillDurationTime, 0, BuffTypes.ATK_PHY, cancelSkill_DRAX30BEffect);
...
//		skill30Eft2Ps.PlayAnim(0);
	...
	protected void showMusic(){
		MusicManager.playEffectMusic("SFX_Drax_Slice&Dice_1a");
	}
	
	protected void cancelSkill_DRAX30BEffect(Character character, Buff self)
	{
		Debug.LogError("cancelSkill_DRBX30BEffect");
		Destroy(skill30BodyEft_Front);
		Destroy(skill30BodyEft_Behind);
		this.drax.attackAnimaName = "Attack";
		Drax drax = character.GetComponent<Drax>();
		drax.showSkill30BMusicCallback -= showMusic;
	}
}
