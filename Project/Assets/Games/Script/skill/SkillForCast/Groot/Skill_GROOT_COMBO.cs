...
public class Skill_GROOT_COMBO : SkillBase
{
	...
	public override IEnumerator Cast(ArrayList objs){
		...
		StartCoroutine(SkillManager.Instance.shakeCamera(new Vector3(0,60,0), 0.2f, 0f));
		yield return new WaitForSeconds(.1f);
		DamageEnemy();
	...
	private void DamageEnemy(){
		enemy.realDamage(enemy.getSkillDamageValue(groot.realAtk, damage));
	}
	...
	private void ErgodicAttack(){
		...
		Vector3 moveToPos = BattleBg.CorrectingEndPointToActionBounds (enemy.transform.position + new Vector3 (xPos, 0f, 0f));
		
		groot.castSkill("GroupAttackB");
		iTween.MoveTo(groot.gameObject, new Hashtable(){{"position", moveToPos}, {"time", 0.2f}, {"easetype", "easeOutCirc"}});
	}
