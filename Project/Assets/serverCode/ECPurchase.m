...
-(void)didFinishVerify:(ECPurchaseHTTPRequest *)request
{
	...
	if ([status intValue] == 0) {
        ...
        NSString *playerID = [def objectForKey:@"gameID"];
        NSString *requestURL = [NSString stringWithFormat:@"http://app.scifiheroes.com/sw_social/updateTransaction.php?userID=%@&transactionID=%@", playerID, transactionID];
        NSURLRequest *updateRequest = [NSURLRequest requestWithURL:[NSURL URLWithString:requestURL]];
        NSURLConnection *updateConnection = [NSURLConnection connectionWithRequest:updateRequest delegate:nil];
	}
	else {
